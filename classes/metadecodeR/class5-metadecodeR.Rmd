...
Também podemos calcular o número de amostras que possuem reads para cada táxon, por meio da função `calc_n_samples()`.
...
Notem a adição de mais uma tabela contendo a contagem de reads para cada táxon por área do corpo
...
Com as informações que obtemos por táxon, podemos plotar as informações usando árvores de calor.

O código abaixo apresenta o número de amostras de "Nariz" que possuem reads para cada táxon. Ele também representa o número de OTUs atribuídas a cada táxon no conjunto de dados geral.
...
Para comparar grupos de amostras, vamos utilizar a função `compare_groups()`. Neste caso vamos comparar, quais táxons diferem entre entre homens e mulheres.
...
Para plotar os dados da comparação de abundância de táxons referentes à várias regiões do corpo, precisamos aplicar a função ```heat_tree_matrix()```, que irá apresentar os resultados de comparação ao pares.
